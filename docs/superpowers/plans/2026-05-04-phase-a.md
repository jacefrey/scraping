...
**Spec:** `docs/superpowers/specs/2026-05-03-scraping-design.md` §4 (web-fetch) and §7 (apify-runner). Acceptance gates in §9.5.
...
`docs/superpowers/specs/2026-05-03-scraping-design.md` §4.1 (inside this plugin repo).
...
```markdown
---
name: Scraping plugin Phase A SHIPPED
description: web-fetch and apify-runner leaf skills shipped as part of the scraping Claude Code plugin at github:jacefrey/scraping. See docs/superpowers/plans/2026-05-04-phase-a.md inside that repo for the plan.
type: project
---

**Status:** Phase A shipped YYYY-MM-DD (replace with absolute date when committing this memory).

**Plugin:** `github:jacefrey/scraping`, version 0.1.0 (or whatever the manifest reports).
**Install:** `claude plugin marketplace add jacefrey/scraping; claude plugin install scraping@scraping`. Skills become invokable as `scraping:web-fetch` and `scraping:apify-runner`.

**What ships:**
- `scraping:web-fetch` — N tests pass (replace with actual count). Lives at `<plugin>/skills/web-fetch/` (canonical) and is symlinked at `~/.claude/skills/web-fetch/` for local dev.
- `scraping:apify-runner` — N tests pass. Lives at `<plugin>/skills/apify-runner/` (canonical) and is symlinked at `~/.claude/skills/apify-runner/`.

Both skills cleared their §9.5 acceptance gates (see `<plugin>/docs/superpowers/specs/2026-05-03-scraping-design.md`). Phase B (`webpage-to-md`, `webpage-to-pdf`, `pdf-to-markdown` `merge_provenance` cross-skill contract change) is next; see §9.2 of that spec.

**How to apply:** When a session needs URL→bytes, the plugin install exposes the skill as `scraping:web-fetch`; for ad-hoc Python imports during local dev, import from `~/.claude/skills/web-fetch/` (the symlink) or directly from the plugin repo. Same shape for `apify-runner`. Phase B converters will sit on top of these.
...
```text
- [Scraping plugin Phase A SHIPPED](project_scraping_plugin_phase_a_shipped.md) — web-fetch + apify-runner skills shipped via github:jacefrey/scraping plugin; ready for Phase B.
```
...
- [ ] **1. Spec coverage check.** Walk each section of `docs/superpowers/specs/2026-05-03-scraping-design.md` §4 (web-fetch) and §7 (apify-runner) and confirm each fielded behavior maps to at least one task above. Any gaps fill with new tasks.
